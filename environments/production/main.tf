...
#
# Components:
# 1. VPC & Networking (subnets, routing — 3 AZs for HA)
# 2. S3 Buckets (logs, data, static assets)
# 3. SQS Queue (event queue with DLQ)
# 4. Lambda Function (event processor with reserved concurrency)
#
# NOTE: This environment uses the SAME modules as dev/staging for consistency.
# IMPORTANT: Production has additional safety features:
# - S3 force_destroy = false (controlled by var.force_destroy_s3)
# - Lambda reserved concurrency capped
# - Longer log retention and message retention
# =============================================================================
...
module "logs_bucket" {
  ...
  force_destroy     = var.force_destroy_s3 # default false; only true in apply tests for teardown
  ...
  lifecycle_rules = [
    {
      ...
      enabled = true
      expiration = {
        days = 365 # Keep logs longer in production
      }
  ...
  lifecycle_rules = [
    {
      ...
      noncurrent_version_expiration = {
        days = 730 # Keep 2 years of versions in production
      }
    }
  ]
...
# =============================================================================
# LAMBDA FUNCTION
# Event processor with production-grade settings
# =============================================================================
