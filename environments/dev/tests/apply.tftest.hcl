# =============================================================================
# APPLY TESTS - DEVELOPMENT ENVIRONMENT
# =============================================================================
...
run "apply_vpc_networking" {
  ...
  assert {
    ...
    error_message = "VPC should have CIDR 10.99.0.0/16"
  }
  ...
  assert {
    condition     = aws_internet_gateway.main.id != null
...
run "apply_s3_buckets" {
  ...
  assert {
    condition     = strcontains(module.logs_bucket.bucket_id, "logs")
    error_message = "Logs bucket name should contain 'logs'"
  }

  assert {
    condition     = strcontains(module.data_bucket.bucket_id, "data")
    error_message = "Data bucket name should contain 'data'"
  }

  assert {
    condition     = strcontains(module.static_bucket.bucket_id, "static")
    error_message = "Static bucket name should contain 'static'"
  }
...
run "apply_lambda_function" {
  ...
  assert {
    condition     = output.lambda_log_group != null
    error_message = "Lambda CloudWatch log group should be output"
  }
}
...
run "apply_full_stack" {
  ...
  assert {
    condition     = output.environment_summary.environment == "dev"
